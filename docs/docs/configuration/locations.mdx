...
The `locations` key of a [`Badge Config`](@todo) overrides the default settings for that badge. This
accepts the same options as the addon, and will apply for that specific badge only. If the string
array option is passed, the types of component that will display this badge will be based on the
