...
define its style, content and more - see [customising badges](../customisation)).
...
<Tabs groupId="using" queryString="using">
  <TabItem default label="Using Tags" value="tags">
    ...
    Global tags, that apply to all components and stories unless [overridden](#tag-inheritance), can be set in your Storybook [`preview`](https://storybook.js.org/docs/configure#configure-story-rendering) config:
    ...
    Component tags will be merged with global tags, unless [overridden](#overriding-tags).
  ...
  <TabItem default label="Using Parameters" value="badges">
    ...
    You can define badges, and the addon configuration using [parameters](https://storybook.js.org/docs/api/parameters).
    ...
    <Tabs groupId="config" queryString="config">
      <TabItem default label="Manager Config" value="manager">
        To add badges to all stories, you can define a `badges` key in your [`manager`](https://storybook.js.org/docs/configure/user-interface/features-and-behavior) config. This will allow these badges to apply to all components/stories, regardless of whether they have their own defined.
      ...
      <TabItem label="Preview Config" value="preview">
        You can use the `preview` configuration file, and set the `badges` parameter.
...
The only exception to this is if you define `global` badges in the `manager` file, in which case
they are available to all components/stories even if they have their own `badges` parameter. If you
wish to override a global badge defined in this way, you can use the same method as
[overriding tags](#tag-inheritance).
